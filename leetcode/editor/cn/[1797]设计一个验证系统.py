...
class AuthenticationManager:
    ...
    def renew(self, tokenId: str, currentTime: int) -> None:
        if self.token_dict.get(tokenId):
            if self.token_dict[tokenId] + self.live_time > currentTime:
                self.token_dict[tokenId] = currentTime
            else:
                self.token_dict.pop(tokenId)

    def countUnexpiredTokens(self, currentTime: int) -> int:
        if len(self.token_dict) == 0:
            return 0
        key_remove = []
        for key, value in self.token_dict.items():
            if value + self.live_time <= currentTime:
                key_remove.append(key)
        for i in key_remove:
            self.token_dict.pop(i)
